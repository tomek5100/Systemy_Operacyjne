...
int main(int argc, char* argv[]){
    ...
    int l = atoi(argv[2]);
    int p = atoi(argv[3]);
     //porownuje wartosc pliku z oczekiwana (potomkowie*sekcje krytyczne)
    printf("Koncowy numer zapisany w pliku wynosi: %d\n",bufor);
    if(bufor == (l*p)){
        printf("Koncowy numer zapisany w pliku zgadza sie z oczekiwanym!\n");
    }
    else
        printf("Koncowy numer zapisany w pliku nie zgadza sie z oczekiwanym!\n");
	
    if(unlink("numer.txt") == -1){ //usuwam plik
        perror("unlink plik error\n");
        exit(EXIT_FAILURE);
    }
