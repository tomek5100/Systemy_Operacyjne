...
int main(int argc, char* argv[], NAMEA, NAMEB, NAMEC, NAMEE){
    
    
    sem_t *semaforA = otwieranie(NAMEA);//otwieram semafor producenta
    ...
    sem_t *semaforB = otwieranie(NAMEB);//otwieram semafor konsumenta
    ...
    typedef struct{ //segment pamieci dzielonej
        ...
        int wstaw = 0; //pozycje wstawiania i wyjmowania z bufora
        int wyjmij = 0;
    }Towar;
    
    
    int fd = mem_open(NAMEC); //otwieram obiekt pamieci dzielonej
...
//    printf("Rozmiar pamieci dzielonej (w bajtach): %d \n",size);
    ...
    if((otworzkopie = open(NAMEE, O_WRONLY|O_CREAT|O_TRUNC, 0644)) == -1){ //tworze plik out.txt
    ...
    men_munmap(adres, size); //usuwanie odwzorowania obszaru pamieci dzielonej
    zamykanie(semaforA);
    zamykanie(semaforB);
