...
const char *name = NULL; //zmienna globalna trzymajaca nazwe dla semafora
...
int main(int argc, char* argv[]){
    ...
    name = "/semafor"; //name trzyma nazwe semafora
    tworzenie(name, 1); //tworze semafor o nazwie "semafor", o wartosci 1
    sem_t *semafor = otwieranie(nazwa);//otwieram semafor
    ...
    for(j=0; j<i; j++){ //tworze argv[2] procesow potomnych
        switch(fork()){
            case -1:
                ...
                exit(EXIT_FAILURE);
            case 0:
                if(execl("./zew.x", "zew.x", argv[3], name, NULL) == -1){
                perror("execl error\n");
                exit(EXIT_FAILURE);
                }
            default:
