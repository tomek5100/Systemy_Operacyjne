...
const char *name = "/semafor"; //zmienna globalna trzymajaca nazwe dla semafora
sem_t *sem;
...
int main(int argc, char* argv[]){
    ...
    //sprawdzic liczbe argumentow
    
    semid adres;
    tworzenie(name, 1); //tworze semafor o nazwie "semafor"
    printf("Adres semafora wynosi: %p\n",adres); //wypisuje adres semafora, %p ponieważ adres jest typu (void *)
    wartosc(adres); //wypisuje informacje o wartosci semafora
    ...
    int otworz;
    if(otworz = open("./numer.txt", O_CREAT|O_TRUNC, 0777) == -1){
